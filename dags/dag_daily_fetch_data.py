...
def fetch_bike_data_to_bq(**context):
    ...
    print(f"📊 Final dataset: {len(df_clean)} records, {len(available_columns)} columns")
    print(f"📊 Sample data:\n{df_clean.head(2)}")
...
def validate_ingestion(**context):
    ...
    records_count = context['ti'].xcom_pull(task_ids='fetch_to_bigquery', key='records_count')
    ...
    if actual_count != records_count:
        print(f"⚠️ WARNING: Record count mismatch (expected {records_count}, got {actual_count})")

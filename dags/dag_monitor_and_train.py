...
def run_drift_monitoring(**context):
    ...
    query = f"""
    SELECT *
    FROM `{ENV_CONFIG['BQ_PROJECT']}.{ENV_CONFIG['BQ_RAW_DATASET']}.daily_*`
    WHERE _TABLE_SUFFIX >= FORMAT_DATE('%Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY))
    LIMIT 1000
    """

    print(f"📥 Loading current data from BigQuery (last 7 days)...")
...
def validate_model(**context):
    ...
    recent_actuals AS (
        ...
        FROM `{ENV_CONFIG['BQ_PROJECT']}.{ENV_CONFIG['BQ_RAW_DATASET']}.daily_*` r
        WHERE _TABLE_SUFFIX >= FORMAT_DATE('%Y%m%d', DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY))
    )
    SELECT
        p.prediction,
    ...
    print(f"📥 Loading predictions and actuals from BigQuery...")
...
def decide_if_fine_tune(**context):
    ...
    if drift and (r2 < R2_THRESHOLD or rmse > RMSE_THRESHOLD):
        ...
        print(f"   → Drift: {drift}")
        print(f"   → R²: {r2:.4f} >= {R2_THRESHOLD}" if r2 >= R2_THRESHOLD else f"   → R²: {r2:.4f}")
        print(f"   → RMSE: {rmse:.2f} <= {RMSE_THRESHOLD}" if rmse <= RMSE_THRESHOLD else f"   → RMSE: {rmse:.2f}")
...
def fine_tune_model(**context):
    ...
    print(f"📥 Fetching fresh training data from BigQuery...")
    ...
    try:
        ...
        context['ti'].xcom_push(key='model_path', value=result.get('model_path', 'unknown'))
...
def end_monitoring(**context):
    ...
    fine_tune_success = context['ti'].xcom_pull(task_ids='fine_tune_model', key='fine_tune_success')
    model_improvement = context['ti'].xcom_pull(task_ids='fine_tune_model', key='model_improvement')
    error_message = context['ti'].xcom_pull(task_ids='fine_tune_model', key='error_message')
    ...
    print(f"   - Fine-tuning: {'✅ SUCCESS' if fine_tune_success else '⛔ NOT TRIGGERED' if not fine_tune_triggered else '❌ FAILED'}")

...
def check_raw_data_exists(**context):
    ...
    raw_table = f"{ENV_CONFIG['BQ_PROJECT']}.{ENV_CONFIG['BQ_RAW_DATASET']}.daily_{today}"
...
def run_daily_prediction(**context):
    ...
    print(f"📥 Reading data from BigQuery...")
    ...
    print(f"📊 Predictions dataset: {len(df_pred)} records, {len(available_pred_columns)} columns")
...
def validate_predictions(**context):
    ...
    """
    pred_table = context['ti'].xcom_pull(task_ids='predict_daily_data', key='pred_table')
    predictions_count = context['ti'].xcom_pull(task_ids='predict_daily_data', key='predictions_count')
    ...
    if actual_count != predictions_count:
        print(f"⚠️ WARNING: Prediction count mismatch (expected {predictions_count}, got {actual_count})")

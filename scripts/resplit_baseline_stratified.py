#!/usr/bin/env python3
"""
Re-split baseline data with stratification by compteur.
Ensures EVERY compteur appears in BOTH train and test sets (ISO split).

This fixes the R² = 0.08 issue caused by unknown compteurs in test set.
"""
...
def load_baseline_data(train_path: str, test_path: str):
    """Load and merge existing train/test baseline"""
    print("=" * 80)
    print("📥 STEP 1: Loading existing baseline data")
    print("=" * 80)

    try:
        df_train = pd.read_csv(train_path, sep=";")
        print(f"✅ Train baseline loaded: {len(df_train):,} rows")
    ...
    try:
        ...
        print(f"✅ Test baseline loaded: {len(df_test):,} rows")
    except Exception as e:
    ...
    print(f"✅ Total merged data: {len(df_full):,} rows")
...
def upload_to_gcs(
    ...
    try:
        ...
        print(
            f"   → You can manually upload the files from {train_path} and {test_path}"
        )
...
def main():
    """Main execution"""
    print("=" * 80)
    print("🚀 BASELINE DATA RE-SPLIT WITH STRATIFICATION")
    print("=" * 80)
    print("Goal: Ensure ALL compteurs present in BOTH train and test (ISO split)")
    print("This fixes R² = 0.08 issue caused by unknown compteurs in test.\n")

    # Load data
    df_full = load_baseline_data(TRAIN_PATH, TEST_PATH)
    ...
    # Save locally
    save_split(df_train, df_test, TRAIN_PATH, TEST_PATH)

    # Upload to GCS
    upload_choice = input(f"\nUpload to GCS gs://{GCS_BUCKET}/ ? (yes/no): ")
    if upload_choice.lower() == "yes":
        upload_to_gcs(TRAIN_PATH, TEST_PATH, GCS_BUCKET, GCS_TRAIN_PATH, GCS_TEST_PATH)
    else:
        print("⏭️  Skipping GCS upload")
        print("   → Manually upload later with:")
        print(f"     gsutil cp {TRAIN_PATH} gs://{GCS_BUCKET}/{GCS_TRAIN_PATH}")
        print(f"     gsutil cp {TEST_PATH} gs://{GCS_BUCKET}/{GCS_TEST_PATH}")

    print("\n" + "=" * 80)
    print("✅ BASELINE RE-SPLIT COMPLETE!")
    print("=" * 80)
    print("Next steps:")
    print(f"1. Train new champion on {TRAIN_PATH}")
    print(f"2. Evaluate on {TEST_PATH} - should see R² ≈ 0.79 (not 0.08!)")
    print("3. Upload new champion to GCS")

...
try:
    ...
    gcp_json_path = os.path.join(tempfile.gettempdir(), "gcp.json")
    with open(gcp_json_path, "w") as f:  # noqa: PTH123
...
def call_prediction_api(url: str, payload: dict, timeout: int = 60):
    try:
        ...
        st.error(
            "⏱️ L’API a mis trop de temps à répondre. Elle est peut-être en cold start."
        )
    except requests.exceptions.ConnectionError:
        st.error(
            "🔌 Impossible de se connecter à l’API. Vérifier l’URL ou la disponibilité du backend."
        )
...
def load_clean_data(path):
    ...
    df["date_et_heure_de_comptage"] = pd.to_datetime(
        df["date_et_heure_de_comptage"], utc=True
    ...
    df[["latitude", "longitude"]] = (
        df["coordonnées_géographiques"].str.split(",", expand=True).astype(float)
    )
...
""")
    ...
    st.image(
        "docs/img/heatmap_static.png",
        caption="Carte des zones à forte affluence cycliste",
        use_container_width=True,
    )
    st.image(
        "docs/img/traffic_hour_day.png",
        caption="Trafic moyen par heure selon le jour de la semaine",
        use_container_width=True,
    )
    st.image(
        "docs/img/pics_barplot.png",
        caption="Top 20 des pics de fréquentation par site de comptage",
        use_container_width=True,
    )
...
""")
    ...
    """)
    df_clean["type_jour"] = df_clean["jour_semaine"].apply(
        lambda x: "weekend" if x in ["Saturday", "Sunday"] else "semaine"
    )
    trafic_jour_type = (
        df_clean.groupby(["type_jour", "heure"])["comptage_horaire"]
        .mean()
        .reset_index()
    )
    plt.figure(figsize=(10, 5))
    sns.lineplot(
        data=trafic_jour_type,
        x="heure",
        y="comptage_horaire",
        hue="type_jour",
        palette="Set1",
    ...
    """)
    top_compteurs = (
        df_clean.groupby("nom_du_compteur")["comptage_horaire"].sum().nlargest(10)
    )
    ...
    """)
    df_time_series = (
        df_clean.groupby(["nom_du_compteur", "date_et_heure_de_comptage"])[
            "comptage_horaire"
        ]
        .mean()
        .unstack(level=0)
    ...
    analyse_pics["Heure_du_pic"] = analyse_pics["Heure_du_pic"].dt.strftime(
        "%Y-%m-%d %H:%M"
    )
    fig = px.bar(
        ...
        x="Compteur",
        y="Trafic_max",
        hover_data=["Heure_du_pic"],
        color="Trafic_max",
        title="<b>Top 20 des pics de trafic cycliste</b>",
    ...
    st.markdown(
        "Le trafic varie sensiblement entre semaine et week-end. Les lundis et vendredis ont des profils distincts."
    )
    df_jour = (
        df_clean.groupby(["jour_semaine", "heure"])["comptage_horaire"]
        .mean()
        .reset_index()
    )
    ordre_jours = [
    ...
    st.write(
        "Nous avons utilisé un RandomForestClassifier pour détecter l’affluence..."
    )
    modele = st.radio(
        "Modèle à utiliser :",
        ["Random Forest", "Réseau de Neurones", "Random Forest Classifier"],
    )
    ...
    Dense(1)  # Sortie
        ...
        st.write(
            "Random Forest Classifier est un modèle d'ensemble qui combine plusieurs arbres de décision. Il est robuste et peut gérer des données manquantes et non linéaires. Il est adapté aux données catégorielles et numériques."
        )
    ...
    **6. Représentation visuelle recommandée**
    ...
    - La **matrice de confusion**, pour visualiser les vrais/faux positifs et négatifs.""")
        st.image(
            "docs/img/matrice_de_confusion.png",
    ...
    **Conclusion :**
    ...
    page = st.sidebar.selectbox(
        "Choisissez une page :", ["🔍 Prédiction exemple", "📂 Prédiction CSV batch"]
    )
            ...
            if result:
                ...
                if model_type == "rf_class":
                    st.success(
                        "📊 Affluence détectée ✅"
                        if pred == 1
                        else "📉 Faible fréquentation attendue"
                    )
                else:
                    # gère les deux cas : [val] ou [[val]]
                    pred = pred[0] if isinstance(pred, (list, tuple)) else pred
                    st.success(
                        f"🧾 Prédiction du comptage horaire : **{round(float(pred))} vélos**"
                    )
            ...
            if st.button("🔁 Forcer ping API"):
                try:
                    ping_response = requests.get(
                        API_URL.replace("/predict", "/docs"), timeout=10
                    )
                    if ping_response.status_code == 200:
                        st.success("✅ API en ligne (endpoint /docs accessible).")
                    else:
                        st.warning(
                            f"⚠️ API répond mais code inattendu : {ping_response.status_code}"
                        )
            ...
            if st.button("🔄 Forcer /refresh_models"):
                try:
                    ...
                    if refresh_response.status_code == 200:
                        ...
                        st.warning(
                            f"⚠️ Requête envoyée mais réponse inattendue : {refresh_response.status_code}"
                        )
                except Exception as e:
    ...
    # === Page CSV ===
        ...
        if uploaded_file is not None:
            ...
            if result:
                ...
                df_csv["prediction_comptage_horaire"] = (
                    predictions.round().astype(int)
                    if model_type != "rf_class"
                    else predictions.astype(int)
                )
